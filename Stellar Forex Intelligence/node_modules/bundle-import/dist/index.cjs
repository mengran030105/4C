...
var importFromString = require('import-from-string');
...
async function moduleFromString(code, options) {
  ...
  if (isESM) {
    return await importFromString.importFromString(code, {
      filename: options.filename,
      dirname: options.dirname,
      skipBuild: false
    });
  } else {
    return importFromString.requireFromString(code, { filename: options.filename, dirname: options.dirname });
  }
}
    ...
    setup(build) {
      ...
      build.onLoad({ filter: JS_EXT_RE }, async (args) => {
        ...
        const injectLines = [
          ...
          `const ${DIRNAME_VAR_NAME} = ${JSON.stringify(node_path.dirname(args.path))};`,
          `const ${IMPORT_META_URL_VAR_NAME} = ${JSON.stringify(node_url.pathToFileURL(args.path).href)};`
        ];
        return {
          contents: injectLines.join("") + contents,
          loader: importFromString.inferLoader(node_path.extname(args.path))
        };
      });
    }
  };
...
function getPkgType() {
  try {
    const pkg = JSON.parse(node_fs.readFileSync(node_path.resolve("package.json"), "utf-8"));
    return pkg.type;
  } catch {
  }
}
function guessFormat(inputFile, type = getPkgType()) {
  const ext = node_path.extname(inputFile);
  if (ext === ".js") {
...
function bundleImport(options) {
  return new Promise((resolve, reject) => {
    ...
    const tsconfig = getTsconfig.getTsconfig(resolved.cwd, options.tsconfig) ?? getTsconfig.getTsconfig(resolved.cwd, "jsconfig.json");
    importFromString.buildBundler({
      ...
      if (result.outputFiles) {
        const absFilePath = node_path.join(resolved.cwd, resolved.filepath);
        const code = result.outputFiles[0].text;
        const mod = await moduleFromString(code, {
          format: resolved.format,
          dirname: node_path.dirname(absFilePath),
          filename: node_path.basename(absFilePath)
        });
        resolve({
          mod,
...
exports.bundleImport = bundleImport;
exports.getPkgType = getPkgType;
exports.guessFormat = guessFormat;
exports.moduleFromString = moduleFromString;
exports.resolveOptions = resolveOptions;
exports.tsconfigPathsToRegExp = tsconfigPathsToRegExp;
