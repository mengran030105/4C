...
function getCallerDirname() {
  ...
  const callSites = new Error("dummy message").stack.filter((callSite) => {
...
async function importFromString(code, options = {}) {
  ...
  try {
    return import(`data:text/javascript;base64,${node_buffer.Buffer.from(result).toString("base64")}`);
  } catch {
    throw new Error(result);
  }
}

function requireFromString(code, options = {}) {
  if (!code.length) {
    throw new Error("code cannot be empty");
  }
  const filename = options?.filename ?? `${Date.now()}.js`;
